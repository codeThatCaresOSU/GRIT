...
class AppDelegate: UIResponder, UIApplicationDelegate, UIAlertViewDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        CoredataManager.sharedInstance.getUserData { (settings: [Setting]?) in
            ...
            if settings!.count > 0 {
                let setting = settings!.last as! Setting
                
                FirebaseManager.sharedInstance.loginUser(email: setting.email!, password: setting.password!, completion: { (user: User) in
                    ...
                    print("Local User Found \(user.email) \(user.firstName )")
                })
            }
